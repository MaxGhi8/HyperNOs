...
def loss_selector(loss_fn_str: str, problem_dim: int, beta: float = 1.0):
    match loss_fn_str.upper():
        ...
        case _:
            raise ValueError(
                "This loss function is not implemented, check the name passed to the loss_selector."
            )
    return loss
...
class LprelLoss_mp(LprelLoss):
    ...
    def abs(
        ...
        norms = torch.pow(
            torch.norm(x.reshape(num_examples, -1), p=self.p, dim=1), self.p
        )
    ...
    def __call__(
        ...
        for i in range(out_dim):
            ...
            for patch in range(n_patches):
                diff += self.abs(
                    x[:, patch, ..., [i]] - y[:, patch, ..., [i]], size_mean=None
                )
    ...
    def __call__(
        ...
        for out in range(out_dim):
            ...
            for i in range(n_patch):
                ...
                # absative L^2 norm
                diff, y_norm = self.abs_errors(x_p, y_p)
                ...
                diff_dx, y_p_dx = self.abs_errors(x_p_dx, y_p_dx)
                ...
                diff_dy, y_p_dy = self.abs_errors(x_p_dy, y_p_dy)
