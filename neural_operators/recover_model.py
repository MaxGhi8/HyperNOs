...
try:
    ...
    raise ValueError("The model is not found, please check the hyperparameters passed trhow the CLI.")
...
#########################################
test_relative_l1, test_relative_l2, test_relative_semih1,\
    test_relative_h1, train_loss = test_fun(model, test_loader, train_loader, loss, exp_norm,
                                            val_samples, training_samples, device, which_example, statistic=True)
...
input_tensor, output_tensor, prediction_tensor, \
    test_rel_l1_tensor, test_rel_l2_tensor, test_rel_semih1_tensor,\
    test_rel_h1_tensor = test_fun_tensors(model, test_loader, loss, device, which_example)
...
match which_example:
    case "fhn" | "fhn_long":
        ...
        prediction_tensor[:, :, [0]] = example.v_normalizer.decode(prediction_tensor[:, :, [0]])
        prediction_tensor[:, :, [1]] = example.w_normalizer.decode(prediction_tensor[:, :, [1]])
    ...
    case "hh":
        ...
        prediction_tensor[:, :, [0]] = example.v_normalizer.decode(prediction_tensor[:, :, [0]])
        prediction_tensor[:, :, [1]] = example.m_normalizer.decode(prediction_tensor[:, :, [1]])
        prediction_tensor[:, :, [2]] = example.h_normalizer.decode(prediction_tensor[:, :, [2]])
        prediction_tensor[:, :, [3]] = example.n_normalizer.decode(prediction_tensor[:, :, [3]])
    ...
    case "crosstruss":
        ...
        prediction_tensor[:, :, :, 0] = (example.max_x - example.min_x)*prediction_tensor[:, :, :, 0] + example.min_x
        prediction_tensor[:, :, :, 1] = (example.max_y - example.min_y)*prediction_tensor[:, :, :, 1] + example.min_y
        # multiplication for domain
        output_tensor[:, :, :, [0]] = output_tensor[:, :, :, [0]] * input_tensor[:, :, :]
        output_tensor[:, :, :, [1]] = output_tensor[:, :, :, [1]] * input_tensor[:, :, :]
        prediction_tensor[:, :, :, [0]] = prediction_tensor[:, :, :, [0]] * input_tensor[:, :, :]
        prediction_tensor[:, :, :, [1]] = prediction_tensor[:, :, :, [1]] * input_tensor[:, :, :]
...
#########################################
# @jaxtyped(typechecker=beartype)  
def test_plot_samples(input_tensor:Float[Tensor, "n_samples *n d_a"], 
                            output_tensor:Float[Tensor, "n_samples *n d_v"], 
                            prediction_tensor:Float[Tensor, "n_samples *n d_v"], 
                            error:Float[Tensor, "n_samples"],
                            mode:str, which_example:str, ntest:int, str_norm:str, n_idx:int=5):
    """ 
    ...
    """
    error, indices = torch.sort(error, descending=True) # Sort the error in descending order
    if mode == "worst":
    ...
    if which_example == "fhn" or which_example == "fhn_long":
        ...
        for i in range(3):
            for j in range(idx.shape[0]):
                if i == 0: # input
                    axs[i, j].plot(x_grid, input_tensor[idx[j], :, :].squeeze(), label='Input (I_app)')
                    if j == 0:
                        axs[i, j].set_ylabel('$I_app$(t)')
                elif i == 1: # v approximation
                    axs[i, j].plot(x_grid, output_tensor[idx[j], :, 0].squeeze(), label='sol')
                    axs[i, j].plot(x_grid, prediction_tensor[idx[j], :, 0].squeeze(), 'r', label='FNO')
                    if j == 0:
                        axs[i, j].set_ylabel('$v$(t)')
                else: # w approximation
                    axs[i, j].plot(x_grid, output_tensor[idx[j], :, 1].squeeze(), label='sol')
                    axs[i, j].plot(x_grid, prediction_tensor[idx[j], :, 1].squeeze(), 'r', label='FNO')
                    if j == 0:
        ...
        for i in range(2):
            for j in range(idx.shape[0]):
                if i == 0: # input
                    ...
                    axs[i, j].plot(output_tensor[idx[j], :, 0].squeeze(), output_tensor[idx[j], :, 1].squeeze(), label='sol')
                    axs[i, j].plot(prediction_tensor[idx[j], :, 0].squeeze(), prediction_tensor[idx[j], :, 1].squeeze(), 'r', label='FNO')
                    if j == 0:
                        axs[i, j].set_ylabel('$w$(t)')
        ...
        for i in range(5):
            for j in range(idx.shape[0]):
                ...
                if i == 0: # input
                    ...
                    axs[i, j].plot(x_grid, output_tensor[idx[j], :, 2].squeeze(), label='sol')
                    axs[i, j].plot(x_grid, prediction_tensor[idx[j], :, 2].squeeze(), 'r', label='FNO')
                    if j == 0:
                        axs[i, j].set_ylabel('$h$(t)')

                elif i == 4: # n approximation
                    axs[i, j].plot(x_grid, output_tensor[idx[j], :, 3].squeeze(), label='sol')
                    axs[i, j].plot(x_grid, prediction_tensor[idx[j], :, 3].squeeze(), 'r', label='FNO')
                    if j == 0:
        ...
        for i in range(7):
            for j in range(idx.shape[0]):
                if i == 0: # input
                    ...
                    error = torch.abs(output_tensor[idx[j], :, :, 0] - prediction_tensor[idx[j], :, :, 0])
                    ...
                    error = torch.abs(output_tensor[idx[j], :, :, 1] - prediction_tensor[idx[j], :, :, 1])
                    im = axs[i, j].imshow(error.squeeze())
                    colorbar = fig.colorbar(im, ax = axs[i, j])
                    if j == 0:
...
def save_tensor(input_tensor, output_tensor, prediction_tensor, which_example:str, norm_str:str): 
        ...
        case "hh":
            data_to_save = {
                ...
                "output": {
                    ...
                    "m": output_tensor[:, :, 1].numpy(),
                    "h": output_tensor[:, :, 2].numpy(),
                    "n": output_tensor[:, :, 3].numpy()
                },
                "predicted": {
                    "V": prediction_tensor[:, :, 0].numpy(),
                    "m": prediction_tensor[:, :, 1].numpy(),
                    "h": prediction_tensor[:, :, 2].numpy(),
                    "n": prediction_tensor[:, :, 3].numpy()
                }
            }
