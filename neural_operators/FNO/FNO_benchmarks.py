...
class ShearLayer:
    def __init__(self, network_properties, device, batch_size, training_samples, in_size = 64, file = "ddsl_N128/", in_dist = True, padding = 4, search_path='/'):
...
class WaveEquationDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples = 1024, t = 5, s = 64, in_dist = True, search_path='/'):
...
def MatReader_darcy(file_path:str) -> tuple[Float[Tensor, "n_samples n_x n_y"], Float[Tensor, "n_samples n_x n_y"]]:
    """
...
class Darcy_Zongyi:
    def __init__(self, network_properties, batch_size, ntrain=1000, ntest=200, s=5, search_path='/'):
...
@jaxtyped(typechecker=beartype)
def MatReader_burgers(file_path:str) -> tuple[Float[Tensor, "n_samples n_x"], Float[Tensor, "n_samples n_x"]]:
    """
...
class Burgers_Zongyi:
    def __init__(self, network_properties, batch_size, ntrain=1000, ntest=200, s=8, search_path='/'):
...
@jaxtyped(typechecker=beartype)
def MatReader_fhn(file_path:str) -> tuple[Float[Tensor, "n_samples n_x"], 
...
class FitzHughNagumo:
    def __init__(self, time:str, network_properties, batch_size, ntrain=3000, ntest=375, s=4, search_path='/'):
        ...
        # Training data
        self.TrainDataPath = find_file(f"Training_dataset_FHN_n_3000_points_5040{time}.mat", search_path)
        a, v, w = MatReader_fhn(self.TrainDataPath)
        a_train, v_train, w_train = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), w[:, ::s].unsqueeze(-1)
        # Compute mean and std (for gaussian point-wise normalization)
        ...
        u_train = torch.concatenate(
            ...
            self.w_normalizer.encode(w_train)), dim=2)

        # Validation data
        self.ValDataPath = find_file(f"Validation_dataset_FHN_n_375_points_5040{time}.mat", search_path)
        a, v, w = MatReader_fhn(self.ValDataPath)
        a_val, v_val, w_val = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), w[:, ::s].unsqueeze(-1)
        a_val = self.a_normalizer.encode(a_val)
        u_val = torch.concatenate(
            (self.v_normalizer.encode(v_val),
            self.w_normalizer.encode(w_val)), dim=2)
        
        # Test data
        self.TestDataPath = find_file(f"Test_dataset_FHN_n_375_points_5040{time}.mat", search_path)
        a, v, w = MatReader_fhn(self.TestDataPath)
        a_test, v_test, w_test = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), w[:, ::s].unsqueeze(-1)
        a_test = self.a_normalizer.encode(a_test)
        u_test = torch.concatenate(
            (self.v_normalizer.encode(v_test),
            self.w_normalizer.encode(w_test)), dim=2)
...
@jaxtyped(typechecker=beartype)
def MatReader_hh(file_path:str) -> tuple[Float[Tensor, "n_samples n_x"], 
                                          Float[Tensor, "n_samples n_x"], 
                                          Float[Tensor, "n_samples n_x"],
                                          Float[Tensor, "n_samples n_x"],
                                          Float[Tensor, "n_samples n_x"]]:
...
class HodgkinHuxley:
    def __init__(self, network_properties, batch_size, ntrain=3000, ntest=375, s=4, search_path='/'):
        # s = 4 --> (5040)//s  = 1260 points
        ...
        # Training data
        self.TrainDataPath = find_file("Training_dataset_HH_n_3000_points_5040_tf_100.mat", search_path)
        a, v, m, h, n = MatReader_hh(self.TrainDataPath)
        a_train, v_train, m_train, h_train, n_train = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), m[:, ::s].unsqueeze(-1), h[:, ::s].unsqueeze(-1), n[:, ::s].unsqueeze(-1)
        # Compute mean and std (for gaussian point-wise normalization)
        self.a_normalizer = UnitGaussianNormalizer(a_train) 
        self.v_normalizer = UnitGaussianNormalizer(v_train)
        ...
        # Validation data
        self.ValDataPath = find_file("Validation_dataset_HH_n_375_points_5040_tf_100.mat", search_path)
        a, v, m, h, n = MatReader_hh(self.ValDataPath)
        a_val, v_val, m_val, h_val, n_val = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), m[:, ::s].unsqueeze(-1), h[:, ::s].unsqueeze(-1), n[:, ::s].unsqueeze(-1)
        a_val = self.a_normalizer.encode(a_val)
        u_val = torch.concatenate(
            (
                self.v_normalizer.encode(v_val),
        ...
        # Test data
        self.TestDataPath = find_file("Test_dataset_HH_n_375_points_5040_tf_100.mat", search_path)
        a, v, m, h, n = MatReader_hh(self.TestDataPath)
        a_test, v_test, m_test, h_test, n_test = a[:, ::s].unsqueeze(-1), v[:, ::s].unsqueeze(-1), m[:, ::s].unsqueeze(-1), h[:, ::s].unsqueeze(-1), n[:, ::s].unsqueeze(-1)
        a_test = self.a_normalizer.encode(a_test)
        u_test = torch.concatenate(
            (
                self.v_normalizer.encode(v_test),
...
class CrossTruss(Dataset):
    def __init__(self, network_properties, batch_size, ntrain=1000, ntest=250, s=2, search_path='/'):
        ...
        # Test set
        inputs_test = inputs[ntrain+ntest : ntrain+2*ntest, ::s, ::s].unsqueeze(-1)
        ...
        outputs_train[:, :, :, 0] = (outputs_train[:, :, :, 0] - self.min_x) / (self.max_x - self.min_x)
        outputs_val[:, :, :, 0] = (outputs_val[:, :, :, 0] - self.min_x) / (self.max_x - self.min_x)
        outputs_test[:, :, :, 0] = (outputs_test[:, :, :, 0] - self.min_x) / (self.max_x - self.min_x)

        self.min_y = torch.min(outputs_train[:, :, :, 1])
        self.max_y = torch.max(outputs_train[:, :, :, 1])
        outputs_train[:, :, :, 1] = (outputs_train[:, :, :, 1] - self.min_y) / (self.max_y - self.min_y)
        outputs_val[:, :, :, 1] = (outputs_val[:, :, :, 1] - self.min_y) / (self.max_y - self.min_y)
        outputs_test[:, :, :, 1] = (outputs_test[:, :, :, 1] - self.min_y) / (self.max_y - self.min_y)
