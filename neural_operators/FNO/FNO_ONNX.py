...
from .FNO_utilities import MatrixIRFFT2, MatrixRFFT2
...
class FourierLayer(nn.Module):
    ...
    ) -> Float[Tensor, "n_batch {self.out_channels} {self.modes} {self.modes}"]:
        """Multiplication between complex numbers"""
    ...
    def forward(
        ...
        if self.problem_dim == 1:
            ...
            irfft2 = MatrixIRFFT2(x.size(-2), x.size(-1), device=x.device)

            # Fourier transform with MatrixRFFT2
            x_ft = rfft2(x)
            ...
            out_ft[:, :, : self.modes, : self.modes, 0] = self.tensor_mul_2d_mod(
                x_ft[:, :, : self.modes, : self.modes, 0], self.weights1[..., 0]
            ) - self.tensor_mul_2d_mod(
                x_ft[:, :, : self.modes, : self.modes, 1], self.weights1[..., 1]
            )

            out_ft[:, :, -self.modes :, : self.modes, 1] = self.tensor_mul_2d_mod(
                x_ft[:, :, -self.modes :, : self.modes, 0], self.weights2[..., 1]
            ) + self.tensor_mul_2d_mod(
                x_ft[:, :, -self.modes :, : self.modes, 1], self.weights2[..., 0]
            )

            # Inverse Fourier transform with MatrixIRFFT2
            x = irfft2(out_ft)
