...
def activation(x:Float[Tensor, "n_samples *n d"], activation_str:str) -> Float[Tensor, "n_samples *n d"]:
    """
...
class MLP_1D(nn.Module):
    ...
    def __init__(self, in_channels:int, out_channels:int, mid_channels:int, fun_act:str):
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_samples d_x d_in"]) -> Float[Tensor, "n_samples d_x d_out"]:
...
class MLP_1D_conv(nn.Module):
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_samples d_in d_x"]) -> Float[Tensor, "n_samples d_out d_x"]:
        x = self.mlp1(x) # affine transformation
        x = activation(x, self.fun_act) # activation function
        x = self.mlp2(x) # affine transformation
        return x
...
class MLP_2D(nn.Module):
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_samples d_x d_y d_in"]) -> Float[Tensor, "n_samples d_x d_y d_out"]:
        x = self.mlp1(x) # affine transformation
        x = activation(x, self.fun_act) # activation function
        x = self.mlp2(x) # affine transformation
        return x
...
class MLP_2D_conv(nn.Module):
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_samples d_in d_x d_y"]) -> Float[Tensor, "n_samples d_out d_x d_y"]:
        x = self.mlp1(x) # affine transformation
        x = activation(x, self.fun_act) # activation function
        x = self.mlp2(x) # affine transformation
        return x
...
class FourierLayer_1D(nn.Module):
    ...
    def __init__(self, in_channels:int, out_channels:int, modes:int,
                 weights_norm:str, fun_act:str, FFTnorm):
    ...
    @jaxtyped(typechecker=beartype)
    def tensor_mul(self, input:Complex[Tensor, "n_batch d_i modes"], 
                         weights:Complex[Tensor, "d_i d_o modes"]) -> Complex[Tensor, "n_batch d_o modes"]:
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_batch d_i n_x"]) -> Float[Tensor, "n_batch d_o n_x"]:
        """ 
        ...
        out_ft[:, :, :self.modes] = \
            self.tensor_mul(x_ft[:, :, :self.modes], self.weights1)
        out_ft[:, :, -self.modes:] = \
            self.tensor_mul(x_ft[:, :, -self.modes:], self.weights2)
...
class FourierLayer_2D(nn.Module):
    ...
    @jaxtyped(typechecker=beartype)
    def tensor_mul(self, input:Complex[Tensor, "n_batch d_i modes1 modes2"], 
                         weights:Complex[Tensor, "d_i d_o modes1 modes2"]) -> Complex[Tensor, "n_batch d_o modes1 modes2"]:
        """ Multiplication between complex numbers """
        # (batch, in_channel, x, y), (in_channel, out_channel, x, y) -> (batch, out_channel, x,y)
        return torch.einsum("bixy,ioxy->boxy", input, weights)

    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_batch d_i n_x n_y"]) -> Float[Tensor, "n_batch d_o n_x n_y"]:
        """ 
        input --> FFT --> parameters --> IFFT --> output  
        Total computation cost is equal to O(n log(n))
        ...
        out_ft[:, :, :self.modes1, :self.modes2] = \
            self.tensor_mul(x_ft[:, :, :self.modes1, :self.modes2], self.weights1)
        out_ft[:, :, -self.modes1:, :self.modes2] = \
            self.tensor_mul(x_ft[:, :, -self.modes1:, :self.modes2], self.weights2)
...
class FNO_1D(nn.Module):
    ...
    """
    def __init__(self, d_a:int, d_v:int, d_u:int, L:int, modes:int, fun_act:str, 
                 weights_norm:str, arc:str='Classic', RNN:bool=False, FFTnorm=None, padding:int=4, 
                 device:torch.device=device, retrain_fno=-1):
        """             
        d_a : int
        ...
        if self.arc == 'Tran': # residual form
            if self.RNN:
                ...
                self.integrals = FourierLayer_1D(self.d_v, self.d_v, self.modes, self.weights_norm, self.fun_act, self.FFTnorm)
            else:
                ...
                self.integrals = nn.ModuleList([
                    FourierLayer_1D(self.d_v, self.d_v, self.modes, self.weights_norm, self.fun_act, self.FFTnorm)
            ...
            if self.RNN:
                ...
                self.mlps = nn.ModuleList([
                    MLP_1D_conv(self.d_v, self.d_v, self.d_v, self.fun_act) for _ in range(self.L) ])
                self.integrals = nn.ModuleList([
                    FourierLayer_1D(self.d_v, self.d_v, self.modes, self.weights_norm, self.fun_act, self.FFTnorm)
                    for _ in range(self.L) ])
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_batch n_x d_a"]) -> Float[Tensor, "n_batch n_x d_u"]:

        ## Grid and initialization
        grid = self.get_grid(x.shape).to(self.device)
        x = torch.cat((grid, x), dim = -1) # concatenate last dimension --> (n_samples)*(n_x)*(d_a+self.problem_dim)
...
class FNO_2D(nn.Module):
    ...
    """
    def __init__(self, d_a:int, d_v:int, d_u:int, L:int, modes1:int, modes2:int, fun_act:str, 
                 weights_norm:str, arc:str='Classic', RNN:bool=False, FFTnorm=None, padding:int=4, 
                 device:torch.device=device, retrain_fno=-1):
        """             
        d_a : int
        ...
        if self.arc == 'Tran': # residual form
            if self.RNN:
                ...
                self.integrals = FourierLayer_2D(self.d_v, self.d_v, self.modes1, self.modes2, self.weights_norm, self.fun_act, self.FFTnorm)
            else:
                self.ws1 = nn.ModuleList([
                ...
                self.ws2 = nn.ModuleList([
                    torch.nn.Conv2d(self.d_v, self.d_v, 1) for _ in range(self.L) ])
                self.integrals = nn.ModuleList([
                    FourierLayer_2D(self.d_v, self.d_v, self.modes1, self.modes2, self.weights_norm, self.fun_act, self.FFTnorm)
                    for _ in range(self.L) ])
            ...
            if self.RNN:
                ...
                self.ws = nn.ModuleList([
                    torch.nn.Conv2d(self.d_v, self.d_v, 1) for _ in range(self.L) ])
                self.mlps = nn.ModuleList([
                    MLP_2D_conv(self.d_v, self.d_v, self.d_v, self.fun_act) for _ in range(self.L) ])
                self.integrals = nn.ModuleList([
                    FourierLayer_2D(self.d_v, self.d_v, self.modes1, self.modes2, self.weights_norm, self.fun_act, self.FFTnorm)
                    for _ in range(self.L) ])
            ...
            if self.RNN:
                ...
                self.ws = nn.ModuleList( [nn.Conv2d(self.d_v, self.d_v, 1) for _ in range(self.L)] )
                self.integrals = nn.ModuleList([
                    FourierLayer_2D(self.d_v, self.d_v, self.modes1, self.modes2, self.weights_norm, self.fun_act, self.FFTnorm)
                    for _ in range(self.L) ])
    ...
    @jaxtyped(typechecker=beartype)
    def forward(self, x:Float[Tensor, "n_batch n_x n_y d_a"]) -> Float[Tensor, "n_batch n_x n_y d_u"]:

        ## Grid and initialization
        grid = self.get_grid(x.shape).to(self.device)
        x = torch.cat((grid, x), dim = -1) # concatenate last dimension --> (n_samples)*(n_x)*(n_y)*(d_a + 2)
