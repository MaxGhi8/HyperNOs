...
def train_fno_lin(which_example: str, mode_hyperparams: str, loss_fn_str: str):
    ...
    hyperparams_train, hyperparams_arc = initialize_hyperparameters(
        "FNO_lin", which_example, mode_hyperparams
    )
    ...
    model_builder = lambda config: FNO_lin(
        config["problem_dim"],
        config["in_dim"],
        config["width"],
        config["out_dim"],
    ...
    experiment_name = (
        f"FNO_lin/{which_example}/loss_{loss_fn_str}_mode_{mode_hyperparams}"
    )
...
if __name__ == "__main__":
    train_fno_lin("coeff_rhs_1d", "default", "L2")
