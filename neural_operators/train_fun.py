...
# Training function
#########################################
def train_fun(model, train_loader, optimizer, scheduler, loss, p:int, device:torch.device,  which_example:str, tepoch=None, n_idx:int=-1):
    """
    ...
    for step, (input_batch, output_batch) in enumerate(train_loader):
        ...
        if which_example == "airfoil":
            ...
            for i in range(input_batch.shape[-1]):
                output_pred_batch[:, :, :, [i]] = output_pred_batch[:, :, :, [i]] * input_batch 
        ...
        try:
            tepoch.set_postfix( {'Batch': step + 1,
                'Train loss (in progress)': train_loss/(input_batch.shape[0]*(step+1))} )
        except:
            pass
...
# Test function
#########################################
def test_fun(model, test_loader, train_loader, loss, exp_norm:str, test_samples:int, 
             training_samples:int, device:torch.device, which_example:str, tepoch=None, statistic=False):
    """
    ...
    with torch.no_grad():
        ...
        for input_batch, output_batch in test_loader:
            ...
            test_relative_l2 += LprelLoss(2, False)(output_pred_batch, output_batch).item()

            # compute the relative semi-H^1 error and H^1 error
            if model.problem_dim == 1:
                test_relative_semih1 += H1relLoss_1D(1.0, False, 0.0)(output_pred_batch, output_batch).item()
                test_relative_h1 += H1relLoss_1D(1.0, False)(output_pred_batch, output_batch).item() # beta = 1.0 in test loss
            elif model.problem_dim == 2:
                test_relative_semih1 += H1relLoss(1.0, False, 0.0)(output_pred_batch, output_batch).item()
                test_relative_h1 += H1relLoss(1.0, False)(output_pred_batch, output_batch).item() # beta = 1.0 in test loss
    ...
    # set the postfix for print
    try:
        tepoch.set_postfix({"Train loss " + exp_norm: train_loss,
    ...
    if statistic:
        return test_relative_l1, test_relative_l2, test_relative_semih1, test_relative_h1, train_loss
    else:
        # return test_relative_l1 # return only the L^1 rel error # !!
        return test_relative_l2 # return only the L^1 rel error

def test_fun_tensors(model, test_loader, loss, device:torch.device, which_example:str):
    ...
    with torch.no_grad():
        ...
        for input_batch, output_batch in test_loader:
            ...
            test_relative_l1_tensor = torch.cat((test_relative_l1_tensor, loss_f), dim=0)

            # compute the relative L^2 error
            loss_f = LprelLoss(2, None)(output_pred_batch, output_batch)
            test_relative_l2_tensor = torch.cat((test_relative_l2_tensor, loss_f), dim=0)

            # compute the relative semi-H^1 error and H^1 error
            if model.problem_dim == 1:
                loss_f = H1relLoss_1D(1.0, None, 0.0)(output_pred_batch, output_batch)
                test_relative_semih1_tensor = torch.cat((test_relative_semih1_tensor, loss_f), dim=0)

                loss_f = H1relLoss_1D(1.0, None)(output_pred_batch, output_batch) # beta = 1.0 in test loss
                test_relative_h1_tensor = torch.cat((test_relative_h1_tensor, loss_f), dim=0)

            elif model.problem_dim == 2:
                loss_f = H1relLoss(1.0, None, 0.0)(output_pred_batch, output_batch)
                test_relative_semih1_tensor = torch.cat((test_relative_semih1_tensor, loss_f), dim=0)

                loss_f = H1relLoss(1.0, None)(output_pred_batch, output_batch) # beta = 1.0 in test loss
                test_relative_h1_tensor = torch.cat((test_relative_h1_tensor, loss_f), dim=0)
            
    return input_tensor, output_tensor, prediction_tensor, test_relative_l1_tensor, test_relative_l2_tensor, test_relative_semih1_tensor, test_relative_h1_tensor