...
match arc:
    ...
    case "CNO":
        hyperparams_train, hyperparams_arc = CNO_initialize_hyperparameters(which_example, mode = mode_str)
    case _:
...
match arc:
    ...
    case "CNO":
        example = CNO_load_data_model(which_example, hyperparams_arc, device, batch_size, training_samples, in_dist)
...
match arc:
    case "FNO":
        if hyperparams_arc["problem_dim"] == 1:
            model = FNO_1D(d_a, d_v, d_u, L, modes, fun_act, weights_norm, fno_arc, 
                            RNN, FFTnorm, padding, device, retrain)    
        elif hyperparams_arc["problem_dim"] == 2:
            model = FNO_2D(d_a, d_v, d_u, L, modes, modes, fun_act, weights_norm, fno_arc, 
                            RNN, FFTnorm, padding, device, retrain)    
    case "CNO":
        if hyperparams_arc["problem_dim"] == 1:
            model = CNO1d(in_dim, out_dim, size, n_layers, n_res, n_res_neck, chan_mul, bn, device)
        elif hyperparams_arc["problem_dim"] == 2:
            model = CNO2d(in_dim, out_dim, size, n_layers, n_res, n_res_neck, chan_mul, bn, device)
...
for epoch in range(epochs):
    with tqdm(desc = f"Epoch {epoch}", bar_format = "{desc}: [{elapsed_s:.2f}{postfix}]") as tepoch:
        ...
        writer.add_scalars(f"FNO_{hyperparams_arc["problem_dim"]}D_" + which_example, {"Train loss " + exp_norm: train_loss,
