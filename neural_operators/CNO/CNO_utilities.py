...
from CNO.CNO_benchmarks import Airfoil, DiscContTranslation, ContTranslation, AllenCahn, SinFrequency, WaveEquation, ShearLayer, Darcy
...
#########################################
def CNO_load_data_model(which_example:str, cno_architecture, device, batch_size, training_samples, in_dist, search_path:str='/'):
    """
    Function to load the data and the model.
    """
    match which_example:
        case "shear_layer":
            example = ShearLayer(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "poisson":
            example = SinFrequency(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "wave_0_5":
            example = WaveEquation(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "allen":
            example = AllenCahn(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "cont_tran":
            example = ContTranslation(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "disc_tran":
            example = DiscContTranslation(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "airfoil":
            example = Airfoil(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
        case "darcy":
            example = Darcy(cno_architecture, device, batch_size, training_samples, in_dist=in_dist, search_path=search_path)
