...
def main():
    ...
    x_data = torch.from_numpy(np.load(find_file("AC_data_input.npy", '/'))).type(torch.float32)
    y_data = torch.from_numpy(np.load(find_file("AC_data_output.npy", '/'))).type(torch.float32)
